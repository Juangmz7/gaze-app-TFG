...
public class CollabClosedEventPublisher implements EventPublisher {
    ...
    public boolean supports(String eventType) {
        return CollabClosedEvent.class.getSimpleName().equals(eventType);
    }

    @Override
    public void publish(OutboxEvent outboxEvent) {
        var event = jsonMapper.fromJson(outboxEvent.getPayload(), CollabClosedEvent.class);
        rabbitTemplate.convertAndSend(
                rabbitMQProperties.getExchange().getPost().getEvents(),
                rabbitMQProperties.getRk().getPost().getCollab().getClosed(),
                event
        );
    }
}
