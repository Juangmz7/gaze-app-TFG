...
public class CollabLinkedEventPublisher implements EventPublisher {
    ...
    public boolean supports(String eventType) {
        return CollabLinkedEvent.class.getSimpleName().equals(eventType);
    }

    @Override
    public void publish(OutboxEvent outboxEvent) {
        var event = jsonMapper.fromJson(outboxEvent.getPayload(), CollabLinkedEvent.class);
        rabbitTemplate.convertAndSend(
                rabbitMQProperties.getExchange().getPost().getEvents(),
                rabbitMQProperties.getRk().getPost().getCollab().getLinked(),
                event
        );
    }
}
