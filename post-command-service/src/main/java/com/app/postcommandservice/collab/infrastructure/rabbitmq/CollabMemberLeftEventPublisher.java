...
public class CollabMemberLeftEventPublisher implements EventPublisher {
    ...
    public boolean supports(String eventType) {
        return CollabMemberLeftEvent.class.getSimpleName().equals(eventType);
    }

    @Override
    public void publish(OutboxEvent outboxEvent) {
        var event = jsonMapper.fromJson(outboxEvent.getPayload(), CollabMemberLeftEvent.class);
        rabbitTemplate.convertAndSend(
                rabbitMQProperties.getExchange().getPost().getEvents(),
                rabbitMQProperties.getRk().getPost().getCollab().getMember().getLeft(),
                event
        );
    }
}
