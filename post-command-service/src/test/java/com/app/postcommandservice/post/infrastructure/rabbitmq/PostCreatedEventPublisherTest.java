...
class PostCreatedEventPublisherTest {
    ...
    @InjectMocks
    private PostCreatedEventPublisher publisher;

    @Test
    void shouldPublishDomainEventToRabbitMqUsingTheConfiguredRoutingKey() {
        var payload = PostCreatedEvent.builder()
                .id(UUID.randomUUID())
        ...
        when(jsonMapper.fromJson("{json}", PostCreatedEvent.class)).thenReturn(payload);
        ...
        when(postRk.getCreated()).thenReturn("rk.post.created");

        publisher.publish(outboxEvent);

        verify(rabbitTemplate).convertAndSend("x.post.events", "rk.post.created", payload);
    }
}
