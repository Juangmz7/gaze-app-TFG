...
class DispatchValidateCommentLikeCommandUseCaseTest {
    ...
    @Captor
    private ArgumentCaptor<ValidateCommentLikeCommand> commandCaptor;
    ...
    @Test
    void shouldSavePendingValidateCommentLikeCommandOutboxRowAndPublishLocalDomainEvent() {
        var postId = UUID.randomUUID();
        var commentId = UUID.randomUUID();
        var userId = UUID.randomUUID();
        var source = CommentLikeSource.SEARCH;
        var feedPosition = 9;
        when(jsonMapper.toJson(commandCaptor.capture())).thenReturn("serialized-comment-like-command");

        dispatchValidateCommentLikeCommandUseCase.dispatch(postId, commentId, userId, source, feedPosition);
        ...
        assertThat(outboxEvent.getPayload()).isEqualTo("serialized-comment-like-command");
        assertThat(outboxEvent.getEventType()).isEqualTo(ValidateCommentLikeCommand.class.getSimpleName());
