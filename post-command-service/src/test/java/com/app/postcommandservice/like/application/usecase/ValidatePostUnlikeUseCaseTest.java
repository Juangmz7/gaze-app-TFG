...
class ValidatePostUnlikeUseCaseTest {
    ...
    void shouldDeleteLikeAndPublishPostLikeDeletedEventWhenCommandIsValid() {
        ...
        when(postLikeEventMapper.toPostLikeDeletedEvent(any(UUID.class), eq(CORRELATION_ID), eq(POST_ID), eq(USER_ID),
    ...
    private ValidatePostUnlikeCommand command() {
        return new ValidatePostUnlikeCommand(COMMAND_ID, CORRELATION_ID, Instant.now(), POST_ID, USER_ID);
    }
}
