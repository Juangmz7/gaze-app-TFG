...
class CollabJoinRequestDeclinedEventPublisherTest {
    ...
    @InjectMocks
    private CollabJoinRequestDeclinedEventPublisher publisher;

    @Test
    void shouldPublishCollabJoinRequestDeclinedEventToRabbitMqUsingConfiguredRoutingKey() {
        var payload = CollabJoinRequestDeclinedEvent.builder()
                .id(UUID.randomUUID())
                .correlationId(UUID.randomUUID())
                .occurredAt(Instant.now())
                .collabId(UUID.randomUUID())
                .userId(UUID.randomUUID())
                .declinedBy(UUID.randomUUID())
        ...
        when(jsonMapper.fromJson("{json}", CollabJoinRequestDeclinedEvent.class)).thenReturn(payload);
        ...
        when(requestRk.getDeclined()).thenReturn("rk.post.collab.request.declined");

        publisher.publish(outboxEvent);

        verify(rabbitTemplate).convertAndSend("x.post.events", "rk.post.collab.request.declined", payload);
    }
}
