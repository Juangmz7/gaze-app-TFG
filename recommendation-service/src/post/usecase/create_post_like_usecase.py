...
from pipeline.repository import post_features_repository
...
class CreatePostLikeUsecase:
    ...
    def execute(self, command: CreatePostLikeCommand) -> None:
        ...
        post_tags_features = self.post_tag_features_repository.getPostsTagsFeatures(command.post_id, post_features.tags)
        if post_tags_features is None:
            raise PostTagFeaturesNotFoundException(command.post_id)

        post_raw_interaction_stats = [
            tag_features.raw_interaction_stats for tag_features in post_tags_features
        ].append(user_creator_features.raw_interaction_stats)
        self._update_raw_interaction_stats(user_creator_features.raw_interaction_stats)

        post_decayed_interaction_stats = [
            (tag_features.decayed_interaction_stats, tag_features.last_updated_at)
             for tag_features in post_tags_features
        ].append(user_creator_features.decayed_interaction_stats, user_creator_features.last_updated_at)
        self._update_decayed_interaction_stats(user_creator_features.decayed_interaction_stats, user_creator_features.last_updated_at)
    ...
    def _update_decayed_interaction_stats(decayed_interactions_stats: list[DecayedInteractionStats, datetime]):
        for decayed_interaction_stats, last_updated_at in decayed_interaction_stats:
            decayed_interaction_stats.likes = decayed_interaction_stats.likes * decay(last_updated_at) + 1
