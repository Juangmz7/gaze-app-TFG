...
from pipeline.config.constants import POST_SHARE_WEIGHT
...
class CreatePostShareInteractionUsecase:
    def __init__(
            self,
    ...
    def execute(self, command: CreatePostShareCommand) -> None:
        ...
        if interaction.ever_shared:
            logger.info(
                "Ignoring duplicate post share interaction: post_id=%s, user_id=%s",
                command.post_id,
                command.user_id,
            )
            return
...
CreatePostShareUsecase = CreatePostShareInteractionUsecase
