...
public class UserFollowedEventPublisher implements EventPublisher {
    ...
    public boolean supports(String eventType) {
        return UserFollowedEvent.class.getSimpleName().equals(eventType);
    }

    @Override
    public void publish(OutboxEvent outboxEvent) {
        var exchange = properties.getExchange().getUser().getEvents();
        var routingKey = properties.getRk().getUser().getFollow().getCreated();
        ...
        log.info("Sending follow message to exchange: {}, routingKey: {}, eventId: {}",
                exchange,
                routingKey,
                outboxEvent.getId());
