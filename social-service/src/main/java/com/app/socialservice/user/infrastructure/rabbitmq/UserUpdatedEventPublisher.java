...
public class UserUpdatedEventPublisher implements EventPublisher {

    private final RabbitTemplate rabbitTemplate;
    private final RabbitMQProperties properties;

    public boolean supports(String eventType) {
        return UserUpdatedEvent.class.getSimpleName().equals(eventType);
    }

    public void publish(OutboxEvent outboxEvent) {
        var exchange = properties.getExchange().getUser().getEvents();
        var routingKey = properties.getRk().getUser().getUpdated();
